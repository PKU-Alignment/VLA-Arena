...
BUILTIN_FIXTURES = {
    "table", "main_table", "kitchen_table", "coffee_table", 
    "living_room_table", "study_table", "floor",
}
    ...
    def get_asset_paths(self, object_names: Set[str]) -> List[AssetInfo]:
        ...
        for name in object_names:
            ...
            if name_lower not in self.asset_mapping:
                ...
                found = False
                for source_dir in ['stable_scanned_objects', 'stable_hope_objects', 
                                   'turbosquid_objects', 'articulated_objects']:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if include_scene and scene_info.scene_xml_full_path:
                ...
                try:
                    ...
                    if scene_info.textures or scene_info.meshes:
                        print(f"  + Scene assets: {len(scene_info.textures)} textures, {len(scene_info.meshes)} meshes")
                except Exception as e:
    ...
    def upload(
        ...
        try:
            ...
            if '403' in error_str or 'storage' in error_str or 'lfs' in error_str:
                print(colored(f"\n⚠ API upload failed: {str(e)}", 'yellow'))
...
Examples:
  ...
  python -m vla_arena.vla_arena.utils.asset_manager download my_task --repo username/task-assets --install
    ...
    upload_parser.add_argument('--repo', required=True, help='HuggingFace repo ID (e.g., username/task-assets)')
    ...
    upload_parser.add_argument('--use-git', action='store_true', help='Use Git LFS instead of API (more reliable for large files)')
    ...
    download_parser.add_argument('--repo', required=True, help='HuggingFace repo ID (e.g., username/task-assets)')
    ...
    list_parser.add_argument('--repo', required=True, help='HuggingFace repo ID (e.g., username/task-assets)')
