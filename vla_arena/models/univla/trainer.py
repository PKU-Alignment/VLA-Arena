...
from vla_arena.models.univla.prismatic.util.data_utils import (
    PaddedCollatorForActionPrediction_VLA_ARENA,
)
from vla_arena.models.univla.prismatic.vla.action_tokenizer import (
    ActionTokenizer,
)
from vla_arena.models.univla.prismatic.vla.datasets import (
...
class Wrapped_Model(torch.nn.Module):
    ...
    def action_decoder_forward(self, batch, vla_output):
        visual_embed = vla_output.hidden_states[-1][
            :, : self.vla.vision_backbone.featurizer.patch_embed.num_patches
        ].to(torch.float)
        latent_tokens = vla_output.hidden_states[-1][
            :, self.vla.vision_backbone.featurizer.patch_embed.num_patches :
        ]
...
def main(config: FinetuneConfig | str | Path) -> None:
    ...
    if isinstance(config, (str, Path)):
        ...
        # Fix: Use config_path
        cfg = draccus.parse(
            FinetuneConfig, config_path=str(config_path), args=[]
        )
    ...
    print(
        f'Config loaded successfully. Dataset: {cfg.dataset_name}, Max Steps: {cfg.max_steps}'
    )
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=int(cfg.max_steps * 0.8), gamma=0.1
    )
    ...
    vla_dataset = RLDSDataset(
        ...
        window_size=cfg.window_size
        + 1,  # for constructing history latent actions
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            batch['latent_action_idx'] = batch['latent_action_idx'].to(
                device_id
            )

            # Forward pass
            output, act_loss, loss_one_step, latent_action_proj = (
                wrapped_model(batch)
            )
            ...
            torch.nn.utils.clip_grad_norm_(
                wrapped_model.parameters(), max_norm=1.0
            )
            ...
            if (
                ...
                if distributed_state.is_main_process:
                    ...
                    torch.save(
                        ...
                        str(run_dir)
                        + f'/action_decoder-{gradient_step_idx}.pt',
                    )
