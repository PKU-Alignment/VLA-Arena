...
class DeviceProcessor:
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        if observation is not None:
            new_observation = {
                k: (
                    v.to(self.device, non_blocking=self.non_blocking)
        ...
        if action is not None and isinstance(action, torch.Tensor):
            new_transition[TransitionKey.ACTION] = action.to(
        ...
        if reward is not None and isinstance(reward, torch.Tensor):
            new_transition[TransitionKey.REWARD] = reward.to(
                self.device, non_blocking=self.non_blocking
            )
        ...
        if done is not None and isinstance(done, torch.Tensor):
            new_transition[TransitionKey.DONE] = done.to(
                self.device, non_blocking=self.non_blocking
            )
