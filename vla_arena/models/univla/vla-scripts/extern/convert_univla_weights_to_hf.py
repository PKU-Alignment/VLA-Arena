...
from vla_arena.models.univla.prismatic.conf import ModelConfig
...
class HFConvertConfig:
    # fmt: off
    openvla_model_path_or_id: str | Path = (                      # Path to Pretrained VLA (on disk or HF Hub)
    ...
    hf_token: str | Path = ''                                     # Environment variable or Path to HF Token
...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    print(
        f'[*] Converting OpenVLA Model `{cfg.openvla_model_path_or_id}` to HF Transformers Format'
    )
    torch.set_default_dtype(torch.bfloat16)

    # Get `config.json`, 'dataset_statistics.json' and `checkpoint_pt` -- mirrors logic in `vla_arena.models.univla.prismatic.models.load.py`
    if os.path.isdir(cfg.openvla_model_path_or_id):
        ...
        config_json, checkpoint_pt = (
            run_dir / 'config.json',
            run_dir / 'checkpoints' / cfg.ckpt_name,
        )
    ...
    # Instantiate & Add Pad to Tokenizer =>> following `vla_arena.models.univla.prismatic.models.materialize.get_llm_backbone_and_tokenizer`
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        '/cpfs01/shared/opendrivelab/qwbu/llama2-7b-hf',
    ...
    assert (
        hf_config.text_config.use_cache
    ), 'LLM config `use_cache` should be True for inference (set default)!'

    # Create Vision Backbone & Transform =>> following `vla_arena.models.univla.prismatic.models.materialize.get_vision_backbone_and_transform`
    ...
    assert ('downsampler' not in model_state_dict) or (
    ...
    # hf_model.language_model.resize_token_embeddings(len(tokenizer))
