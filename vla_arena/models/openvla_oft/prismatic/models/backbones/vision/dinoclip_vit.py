...
class DinoCLIPImageTransform:
    ...
    def __call__(self, img: Image, **kwargs: str) -> dict[str, torch.Tensor]:
        return {
            ...
            'clip': self.clip_image_transform(img, **kwargs),
        }
...
class DinoCLIPViTBackbone(VisionBackbone):
    def __init__(
        ...
        self.clip_featurizer.forward = unpack_tuple(
            partial(
                self.clip_featurizer.get_intermediate_layers,
                n={len(self.clip_featurizer.blocks) - 2},
            )
        )
        ...
        if self.image_resize_strategy == 'resize-naive':
            ...
            ), 'Unexpected `default_dino_image_transform`!'
            assert isinstance(
                default_clip_transform, Compose
            ), 'Unexpected `default_clip_image_transform`!'
            ...
            self.image_transform = DinoCLIPImageTransform(
                default_dino_transform, default_clip_transform
            )

        elif self.image_resize_strategy == 'letterbox':
            assert isinstance(
                default_dino_transform, Compose
            ), 'Unexpected `default_dino_transform`!'
            assert isinstance(
                default_clip_transform, Compose
            ), 'Unexpected `default_clip_transform`!'
            assert (
                'mean' in self.dino_data_cfg and 'mean' in self.clip_data_cfg
    ...
    def num_patches(self) -> int:
        assert (
            ...
            == self.clip_featurizer.patch_embed.num_patches
        )
