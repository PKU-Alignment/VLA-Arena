...
the HuggingFace "AutoClasses" (e.g., those defined in `vla_arena.models.openvla_oft.prismatic.extern.hf_*`) for "native" use in `transformers``
...
def convert_prismatic_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    # Get `config.json` and `checkpoint_pt` -- mirrors logic in `vla_arena.models.openvla_oft.prismatic.models.load.py`
    ...
    # Instantiate & Add Pad to Tokenizer =>> following `vla_arena.models.openvla_oft.prismatic.models.materialize.get_llm_backbone_and_tokenizer`
    ...
    # Create Vision Backbone & Transform =>> following `vla_arena.models.openvla_oft.prismatic.models.materialize.get_vision_backbone_and_transform`
