...
from vla_arena.models.univla.prismatic.models.materialize import (
    get_llm_backbone_and_tokenizer,
    get_vision_backbone_and_transform,
)
from vla_arena.models.univla.prismatic.models.registry import GLOBAL_REGISTRY, MODEL_REGISTRY
from vla_arena.models.univla.prismatic.models.vlas import OpenVLA
...
def get_model_description(model_id_or_name: str) -> str:
    if model_id_or_name not in GLOBAL_REGISTRY:
        raise ValueError(
            f"Couldn't find `{model_id_or_name = }; check `vla_arena.models.univla.prismatic.available_model_names()`"
        )
...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        if model_id_or_path not in GLOBAL_REGISTRY:
            raise ValueError(
                f"Couldn't find `{model_id_or_path = }; check `vla_arena.models.univla.prismatic.available_model_names()`"
            )
...
def load_vla(
    ...
    if os.path.isfile(model_id_or_path):
        overwatch.info(
            f'Loading from local checkpoint path `{(checkpoint_pt := Path(model_id_or_path))}`'
        )
        ...
        config_json, dataset_statistics_json = (
            run_dir / 'config.json',
    ...
    # Otherwise =>> try looking for a match on `model_id_or_path` on the HF Hub (`VLA_HF_HUB_REPO`)
        ...
        overwatch.info(
            f'Checking HF for `{(hf_path := str(Path(VLA_HF_HUB_REPO) / model_type / model_id_or_path))}`'
        )
        ...
        overwatch.info(
            f'Downloading Model `{model_id_or_path}` Config & Checkpoint `{target_ckpt}`'
        )
        with overwatch.local_zero_first():
