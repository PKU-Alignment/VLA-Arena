"""
Random policy evaluator for VLA-Arena.

Uses the shared multi-suite flow and emits JSON metrics.
"""
...
def initialize_model(cfg: EvaluatorConfig) -> np.random.Generator:
...
def get_action(
    ...
    del cfg, observation, task_description
    action = rng.uniform(low=-1.0, high=1.0, size=7)
...
def process_action(action: Sequence[float]) -> Sequence[float]:
    return np.clip(np.array(action), -1.0, 1.0).tolist()
...
def run_task(
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task), desc=f'Task {task_id}'):
        ...
        if _should_save_video(cfg.save_video_mode, success, first_success, first_failure):
            ...
            _save_rollout_video(frames, video_path)
            if success:
                first_success = True
            else:
                first_failure = True
...
def _save_rollout_video(frames: Iterable[np.ndarray], path: pathlib.Path):
    import imageio
...
def main(cfg: EvaluatorConfig | str | pathlib.Path | None = None):
    ...
    rng = initialize_model(cfg)
    log_file, log_path, run_id = setup_logging(cfg)
    log_message(f'Initialized random policy with seed {cfg.seed}', log_file)
    ...
    if cfg.result_json_path is None or str(cfg.result_json_path).lower() == 'default':
        ...
        result_path = result_dir / f'random_json_{DATE_TIME}.json'
    else:
