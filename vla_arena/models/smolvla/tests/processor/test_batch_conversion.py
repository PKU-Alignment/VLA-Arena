...
def test_batch_to_transition_observation_grouping():
    ...
    assert torch.allclose(
        transition[TransitionKey.OBSERVATION]['observation.image.top'],
...
def test_complex_nested_observation():
    ...
    assert torch.allclose(
        batch['observation.image.top']['image'],
        reconstructed_batch['observation.image.top']['image'],
    )
    assert torch.allclose(
        batch['observation.image.left']['image'],
        reconstructed_batch['observation.image.left']['image'],
    )
