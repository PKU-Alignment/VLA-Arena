...
the HuggingFace "AutoClasses" (e.g., those defined in `vla_arena.models.openvla.prismatic.extern.hf_*`) for "native" use in `transformers``
...
from vla_arena.models.openvla.vla_arena.models.openvla.prismatic.extern.hf.modeling_prismatic import (
    PrismaticForConditionalGeneration,
)
from vla_arena.models.openvla.vla_arena.models.openvla.prismatic.extern.hf.processing_prismatic import (
...
class HFConvertConfig:
    # fmt: off
    prismatic_model_path_or_id: str | Path = (                    # Path to Pretrained VLM (on disk or HF Hub)
        'siglip-224px+7b'
...
def convert_prismatic_weights_to_hf(cfg: HFConvertConfig) -> None:
    print(
        f'[*] Converting Prismatic Model `{cfg.prismatic_model_path_or_id}` to HF Transformers Format'
    )
    ...
    if os.path.isdir(cfg.prismatic_model_path_or_id):
        ...
        checkpoint_pt = hf_hub_download(
            ...
            f'{cfg.prismatic_model_path_or_id}/checkpoints/latest-checkpoint.pt',
        )
    ...
    # Instantiate & Add Pad to Tokenizer =>> following `vla_arena.models.openvla.prismatic.models.materialize.get_llm_backbone_and_tokenizer`
    ...
    # Create Vision Backbone & Transform =>> following `vla_arena.models.openvla.prismatic.models.materialize.get_vision_backbone_and_transform`
    ...
    assert ('projector' in model_state_dict) and (
