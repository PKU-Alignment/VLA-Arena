...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(
        ...
        if self.image_resize_strategy == 'resize-naive':
            ...
            assert isinstance(
                default_siglip_transform, Compose
            ), 'Unexpected `default_siglip_image_transform`!'
            ...
            self.image_transform = DinoSigLIPImageTransform(
                default_dino_transform, default_siglip_transform
            )
            ...
            assert isinstance(
                default_siglip_transform, Compose
            ), 'Unexpected `default_siglip_transform`!'
            assert (
    ...
    def forward(self, pixel_values: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if isinstance(pixel_values, dict):
            ...
            dino_patches = self.dino_featurizer(pixel_values[:, :3])
            siglip_patches = self.siglip_featurizer(pixel_values[:, 3:])
