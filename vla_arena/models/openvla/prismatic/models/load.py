...
from vla_arena.models.openvla.prismatic.models.registry import GLOBAL_REGISTRY, MODEL_REGISTRY
from vla_arena.models.openvla.prismatic.models.vlas import OpenVLA
...
def get_model_description(model_id_or_name: str) -> str:
    if model_id_or_name not in GLOBAL_REGISTRY:
        raise ValueError(
            f"Couldn't find `{model_id_or_name = }; check `vla_arena.models.openvla.prismatic.available_model_names()`"
        )
...
def load(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        if model_id_or_path not in GLOBAL_REGISTRY:
            raise ValueError(
                f"Couldn't find `{model_id_or_path = }; check `vla_arena.models.openvla.prismatic.available_model_names()`"
            )
