...
from vla_arena.models.openvla_oft.prismatic.models.backbones.llm.prompting import PurePromptBuilder
...
from vla_arena.models.openvla_oft.prismatic.vla.datasets import RLDSBatchTransform, RLDSDataset
...
class FinetuneConfig:
    # fmt: off
    # Set OPENVLA_OFT_VLA_PATH environment variable to specify a custom VLA model path.
    vla_path: str = os.getenv('OPENVLA_OFT_VLA_PATH', '/path/to/your/openvla-model')             # Path to OpenVLA model (on HuggingFace Hub or stored locally)

    # Dataset
    # Set OPENVLA_OFT_DATA_ROOT_DIR environment variable to specify a custom data root directory.
    data_root_dir: Path = Path(os.getenv('OPENVLA_OFT_DATA_ROOT_DIR', '/path/to/your/rlds-datasets'))      # Directory containing RLDS datasets
    dataset_name: str = 'vla_arena'    # Name of fine-tuning dataset (e.g., `aloha_scoop_x_into_bowl`)
    ...
    resume_step: int | None = None                # (When `resume==True`) Step number that we are resuming from
    ...
    run_id_override: str | None = None            # Optional string to override the run ID with
...
def run_diffusion_sampling(
    ...
    for t in action_head.module.noise_scheduler.timesteps:
        ...
        diffusion_timestep_embeddings = (
            action_head.module.time_encoder(timesteps)
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        if (cfg.use_l1_regression or cfg.use_diffusion) and action_head is not None:
            torch.save(
                action_head.state_dict(), checkpoint_dir / f'action_head--{checkpoint_name_suffix}'
            )
...
def main(config: FinetuneConfig | str | Path) -> None:
    """
    ...
    Args:
        config (Union[FinetuneConfig, str, Path]): Training configuration or path to config file.
    ...
    if isinstance(config, (str, Path)):
        ...
        # Fix: Use config_path
        cfg = draccus.parse(FinetuneConfig, config_path=str(config_path), args=[])

    elif isinstance(config, FinetuneConfig):
        cfg = config
    else:
        raise ValueError(
            f'Unsupported config type: {type(config)}. Expected FinetuneConfig or path string.'
        )

    # Test print to ensure configuration is loaded
    print(f'Config loaded successfully. Dataset: {cfg.dataset_name}, Max Steps: {cfg.max_steps}')
    ...
    # the `modeling_vla_arena.models.openvla_oft.prismatic.py` file in this codebase; if so, we will copy
    ...
    if cfg.use_diffusion:
        trainable_params += [
            param for param in noisy_action_projector.parameters() if param.requires_grad
        ]
    if cfg.use_proprio:
        trainable_params += [
    ...
    #
    # ---
    # from vla_arena.models.openvla_oft.prismatic.vla.datasets import DummyDataset
    #
