...
VLA-Arena 提供了一套用于在自定义的场景中搜集数据并转换搜集到的数据格式的完整框架。本指南将帮助你了解如何在已有的场景中搜集数据，并将其转换为可用的数据集。
...
你需要操作机械臂完成 BDDL 文件中 `language instruction` 给出的任务，同时避免产生任何 BDDL 文件中所定义的 `cost`。
...
修改 `VLA_Arena/VLA_Arena_dataset_builder.py` 文件中的 `_split_paths()` 方法，将数据路径指向你的重构数据集：
