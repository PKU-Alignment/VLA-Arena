    ...
    for buffer_property in dict_properties():
        for k in got_batch_transition[buffer_property]:
            ...
            for got_state_item in got_state:
                assert any(
                    torch.equal(got_state_item, dummy_state[k]) for dummy_state in dummy_states
        ...
        for feature, value in ds[i].items():
            if feature == 'action':
                ...
                torch.testing.assert_close(
                    value, buffer.states['observation.image'][i], rtol=0.3, atol=0.003
                )
...
def test_check_image_augmentations_with_drq_and_dummy_image_augmentation_function(
    ...
    assert torch.all(
        sampled_transitions['state']['observation.image'] == 10
    ), 'Image augmentations should be applied'
    assert torch.all(
        sampled_transitions['next_state']['observation.image'] == 10
    ), 'Image augmentations should be applied'
